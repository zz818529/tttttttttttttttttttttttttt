...
async def logger_id(client):
    desc = "A Bot Logger Group For ★★ᴅᴀʀᴋ ᴍᴏᴏɴ ᴜꜱᴇʀʙᴏᴛ ★★ . DO NOT LEAVE THIS GROUP!!"
    try:
        grp = await client(
            functions.channels.CreateChannelRequest(
                title="★★ᴅᴀʀᴋ ᴍᴏᴏɴ ᴜꜱᴇʀʙᴏᴛ ★★   Logger", about=desc, megagroup=True
            )
        )
...
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@itzyournil"))
